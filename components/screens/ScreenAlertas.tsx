...
export const ScreenAlertas: React.FC<Props> = ({ data }) => {
  const { critical, warning, ok } = useMemo(() => {
    ...
    return {
      ...
      ok:       sorted.filter(b => b.inactiveMinutes <= 60 && b.hoyNeto > 0),
    };
  }, [data.branches]);
  ...
  const BranchRow: React.FC<{ b: BranchData; accent: string; pulse?: boolean }> = ({ b, accent, pulse }) => (
    <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-white/3 border border-white/5">
      ...
      <div className="flex items-center gap-6 shrink-0">
        <span className="text-gray-500 text-xs font-mono">último: {b.ultimaHoraTicket || 'sin datos'}</span>
        <span className="font-mono font-bold text-sm px-2 py-0.5 rounded" style={{ color: accent, background: accent + '22' }}>
  ...
  return (
    <div className="w-screen h-screen bg-[#0b0e14] text-white flex flex-col p-8 overflow-hidden">
      <div className="mb-6 shrink-0 flex items-end justify-between border-b border-white/5 pb-4">
        <div>
          <p className="text-[#C8102E] text-xs font-bold tracking-[0.3em] uppercase mb-1">Sin ticket en la última hora</p>
        ...
        <div className="flex gap-6">
          <div className="text-center">
            ...
            <p className="text-gray-500 text-xs font-bold tracking-widest">+2hs</p>
          </div>
          <div className="text-center">
            <p className="text-yellow-400 font-mono font-black text-3xl">{warning.length}</p>
            <p className="text-gray-500 text-xs font-bold tracking-widest">1-2hs</p>
          </div>
          <div className="text-center">
            <p className="text-[#01B693] font-mono font-black text-3xl">{ok.length}</p>
            <p className="text-gray-500 text-xs font-bold tracking-widest">OK</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-2 gap-x-8 overflow-hidden">
        <div className="flex flex-col overflow-hidden pr-2 gap-3">
          {critical.length > 0 && (
            <div>
              <p className="text-[#C8102E] text-xs font-bold tracking-widest uppercase mb-2">Crítico (+2hs)</p>
          ...
          {warning.length > 0 && (
            <div>
              <p className="text-yellow-400 text-xs font-bold tracking-widest uppercase mb-2">Advertencia (1-2hs)</p>
              <div className="flex flex-col gap-1.5">
                {warning.map(b => <BranchRow key={b.id} b={b} accent="#eab308" />)}
              </div>
            </div>
          )}
          {critical.length === 0 && warning.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-[#01B693] font-bold text-lg">Sin alertas críticas</p>
              <p className="text-gray-500 text-sm mt-1">Todas las sucursales operando con normalidad</p>
            </div>
          )}
        </div>
