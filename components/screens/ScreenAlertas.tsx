...
const AlertRow: React.FC<{ b: BranchData; accent: string; pulse?: boolean }> = ({ b, accent, pulse }) => (
        ...
        <div className="flex flex-col overflow-hidden pr-2 gap-3">
          ...
          {warning.length > 0 && (
            <div>
              ...
              <div className="flex flex-col gap-1.5">
                {warning.map(b => <AlertRow key={b.id} b={b} accent="#f59e0b" />)}
              </div>
            </div>
          )}
        ...
        <div className="flex flex-col overflow-hidden pl-2 border-l border-white/5">
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            <AutoScrollList
              items={ok}
              itemHeight={52}
              renderItem={(b) => <AlertRow key={b.id} b={b} accent="#01B693" />}
            />
          </div>
        </div>
      </div>
